                   ...
                   scale.estimator, psifun, ...)
{
  ...
  # m <- findfrequency(y*100)
  m <- find_frequency(y*100)
  ...
  if(class(model)=="robets")
  {
    ...
    {
      print('it did not enter to the use initial values conditional')
      ...
      for(i3 in 1:length(seasontype))
      {
        for(i4 in 1:length(damped))
        {
          ...
          if(additive.only & (errortype[i1]=="M" | seasontype[i3]=="M"))
            next
          fit <- robetsmodel(y,errortype[i1],trendtype[i2],seasontype[i3],damped[i4],alpha,beta,gamma,phi,lower=lower,upper=upper,opt.crit=opt.crit,bounds=bounds,opt.initial.values=opt.initial.values,rob.start.initial.values=rob.start.initial.values,opt.sigma0=opt.sigma0,k=k,nmse=nmse,...,scale.estimator = scale.estimator, psifun = psifun)
...
robpegelsresid.C <- function(y,m,initstate,errortype,trendtype,seasontype,damped,alpha,beta,gamma,phi,nmse,k)
{
  ...
  res <- .Call("robets_calc_out", y, m, initstate, switch(errortype, "A" = 1, "M" = 2),
   switch(trendtype, "N" = 0, "A" = 1, "M" = 2), 
   switch(seasontype,"N" = 0,"A" = 1, "M" = 2),
   damped, alpha, beta, gamma, phi, nmse, k, PACKAGE = "robets")
  if(!is.na(res$lik)){
  ...
  return(list(lik=res$lik,amse=res$amse[1:nmse],roblik=res$roblik,tau2=res$tau2, e=res$e,
    states=matrix(res$states, nrow=length(y)+1, ncol=(2+(trendtype!="N")+m*(seasontype!="N")), byrow=TRUE),
    y_pred=res$y_pred))
}
                        ...
                        psifun)
{
  ...
  if(np >= length(y)-1) # Not enough data to continue
    return(list(roblik=Inf,robaic=Inf,robbic=Inf,robaicc=Inf,
                ...
                amse=Inf,tau2=Inf,fit=NULL,states=initstate,
                par=par, y_pred=Inf))
  ...
  env <- robetsTargetFunctionInit(par=par, y=y, errortype=errortype, trendtype=trendtype,
                                  ...
                                  opt.sigma0=opt.sigma0, opt.initial.values = opt.initial.values, optK = optK, psifun = psifun)
  ...
  return(list(roblik=roblik,robaic=robaic,robbic=robbic,robaicc=robaicc,loglik=loglik,aic=aic,bic=bic,aicc=aicc,mse=mse,
  amse=amse,tau2=tau2,fit=fred,residuals=ts(e$e,frequency=tsp.y[3],start=tsp.y[1]),
  fitted=ts(fits,frequency=tsp.y[3],start=tsp.y[1]),states=states,par=fit.par, y_pred=y_pred))
}

robetsTargetFunctionInit <- function(par,y,errortype,trendtype,seasontype,damped,par.noopt,lowerb,upperb,
                                     opt.crit,nmse,bounds,m,pnames,pnames2,opt.sigma0,opt.initial.values,optK, psifun)
{
  names(par) <- pnames
  ...
  res <- .Call("robetsTargetFunctionInit", y=y, errortype=switch(errortype,"A"=1,"M"=2),
               ...
               alpha, beta, gamma, phi,sigma0,initstate,k, psifun, env, PACKAGE = "robets")
  res
}
