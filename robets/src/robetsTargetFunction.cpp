...
void RobetsTargetFunction::robetscalc(){
  ...
  for (i=0; i<n; i++){
        ...
        if(errortype==ADD){
          ...
          }else if (psifun=="Bisquare"){
            ydown = sigma*psibisquare(e[i]/sigma) + f[0];
          }else if (psifun=="Hampel"){
            ydown = sigma*psihampel(e[i]/sigma) + f[0];
          }else if (psifun=="Welsh1"){
            ydown = sigma*psiwelsh1(e[i]/sigma) + f[0];
          }else if (psifun=="Welsh2"){
            ydown = sigma*psiwelsh2(e[i]/sigma) + f[0];
          }else{
            ydown = sigma*psihuber(e[i]/sigma) + f[0];
          }
        }else{
          if (psifun=="Huber"){
            ...
            ydown = f[0] * sigma*psibisquare(e[i]/sigma) + f[0];
          }else if (psifun=="Hampel"){
            ydown = f[0] * sigma*psihampel(e[i]/sigma) + f[0];
          }else if (psifun=="Welsh1"){
            ydown = f[0] * sigma*psiwelsh1(e[i]/sigma) + f[0];
          }else if (psifun=="Welsh2"){
            ydown = f[0] * sigma*psiwelsh2(e[i]/sigma) + f[0];
          }else{
            ydown = f[0] * sigma*psihuber(e[i]/sigma) + f[0];
          }
        }
        ydownlist.push_back(ydown);
...
// BISQUARE
double RobetsTargetFunction::psibisquare(double x){
  if(std::abs(x)<=k){
    return x*pow(1-pow(x/k,2),2);
  }else{
    return 0;
  }
}
...
// HAMPEL
double RobetsTargetFunction::psihampel(double x){
  double a = 1.5*k;
  double b = 3.5*k;
  double r = 8*k;
  if(std::abs(x)<=a){
   return x;
  }else if (std::abs(x)<=b && std::abs(x) > a){
   return copysign(a,x);
  }else if (std::abs(x)<=r && std::abs(x) > b){
   return copysign(a,x) * ((r-std::abs(x))/(r-b));
  }else{
   return 0;
  }
}
...
// WELSH 1
double RobetsTargetFunction::psiwelsh1(double x){
  return x * exp(-pow(x/k,2)/2);
}

// WELSH 2
double RobetsTargetFunction::psiwelsh2(double x){
  return (1 - pow(x/k,2)) * exp(-pow(x/k,2)/2);
}
